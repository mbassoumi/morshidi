...
import {CheckboxField, DatePickerField, InputField, SelectField, StyledButton} from '../../shared/formik/FormikComponents';
...
const partialDefaultClassSetting = {
    minStudentPerClass: Yup.number()
        .min(1, 'Min students number per group_class can\'t be empty group_class')
        .max(Yup.ref('maxStudentPerClass'), 'Min students number per group_class can\'t be grater than max value')
        .required('Min students number per group_class is required'),
    maxStudentPerClass: Yup.number()
        .min(Yup.ref('minStudentPerClass'), 'Max students number per group_class can\'t be less than min value')
        .required('Max students number per group_class is required'),
...
const GroupClassForm = ({defaultValues, serverErrors, onSubmit, courseName, keywords, fields, cities, levels}: any) => {
